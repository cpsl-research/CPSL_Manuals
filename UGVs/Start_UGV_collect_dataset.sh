...
if [ $? != 0 ]; then
    ...
    tmux send-keys -t $SESSION_NAME:0.0 "ros2 service call /cpsl_ugv_1/reset_pose irobot_create_msgs/srv/ResetPose \"pose: {position: {x: 0, y: 0, z: 0}, orientation: {x: 0, y: 0, z: 0, w: 1}}\"" C-m
    sleep 5
    echo "pose reset complete"
    ...
    tmux send-keys -t $SESSION_NAME:0.3 "ros2 launch cpsl_ros2_sensors_bringup ugv_sensor_bringup_ragnnarok.launch.py lidar_enable:=true lidar_scan_enable:=true camera_enable:=true radar_enable:=true platform_description_enable:=true rviz:=false namespace:=cpsl_ugv_1" C-m
    sleep 30
    ...
    tmux send-keys -t $SESSION_NAME:0.5 "ros2 launch cpsl_nav slam_sync.launch.py slam_params_file:=slam_athena.yaml scan_topic:=/livox/scan namespace:=$PLATFORM_NAME base_frame_id:=base_link" C-m
    sleep 30
    echo "SLAM setup complete"
    ...
    tmux send-keys -t $SESSION_NAME:0.6 "ros2 service call /cpsl_ugv_1/slam_toolbox/save_map slam_toolbox/SaveMap \"{name: {data: 'cpsl_map'}}\""
    ...
    tmux send-keys -t $SESSION_NAME:0.7 "ros2 launch dataset_generator record_dataset.launch.py namespace:=$PLATFORM_NAME param_file:=ugv_dataset.yaml dataset_subpath:=north_1"
