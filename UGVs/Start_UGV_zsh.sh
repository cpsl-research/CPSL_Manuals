...
if [ $? != 0 ]; then
    ...
    tmux send-keys -t $SESSION_NAME:0.1 "cd CPSL_ROS2_Create3 && source install/setup.zsh && ros2 launch create3_bringup create3_bringup.launch.py namespace:=cpsl_ugv_1" C-m

    echo "Starting keyop controller..."
    tmux send-keys -t $SESSION_NAME:0.2 "cd CPSL_ROS2_Create3 && source install/setup.zsh && ros2 run teleop_twist_keyboard teleop_twist_keyboard --ros-args --remap /cmd_vel:=/cpsl_ugv_1/cmd_vel" C-m
