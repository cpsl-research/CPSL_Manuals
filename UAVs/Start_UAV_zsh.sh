...
if [ $? != 0 ]; then
    ...
    tmux send-keys -t $SESSION_NAME:0.1 "ros2 launch px4_controller joy_control_launch.py joy_enable:=true control_enable:=true namespace:=cpsl_uav_1" C-m
    ...
    echo "setting up altitude estimation..."
    ...
    tmux send-keys -t $SESSION_NAME:0.3 "ros2 launch mmwave_radar_processing_ros altitude_estimator_bringup.launch.py"

    echo "setting up velocity estimation..."
    tmux send-keys -t $SESSION_NAME:0.4 "cd CPSL_ROS2_RadStack" C-m
    sleep 1
    ...
    tmux send-keys -t $SESSION_NAME:0.4 "ros2 launch mmwave_radar_processing_ros velocity_estimator_points_bringup.launch.py"

    echo "setting up PC Processing (GNN)"
    tmux send-keys -t $SESSION_NAME:0.5 "cd CPSL_ROS2_PCProcessing && poetry shell" C-m
    sleep 1
    tmux send-keys -t $SESSION_NAME:0.5 "source install/setup.zsh" C-m
    sleep 1
    tmux send-keys -t $SESSION_NAME:0.5 "ros2 launch pc_processing ugv_bringup.launch.py scan_enable:=true namespace:=cpsl_ugv_1" 

    echo "setting up SLAM (lidar for now)"
    tmux send-keys -t $SESSION_NAME:0.6 "cd CPSL_ROS2_Nav" C-m
    tmux send-keys -t $SESSION_NAME:0.6 "source install/setup.zsh" C-m
    tmux send-keys -t $SESSION_NAME:0.6 "ros2 launch cpsl_nav slam_sync.launch.py slam_params_file:=slam_athena.yaml scan_topic:=/livox/scan namespace:=cpsl_uav_1 base_frame_id:=base_footprint"
    ...
    tmux send-keys -t $SESSION_NAME:0.7 "ros2 launch cpsl_nav nav2.launch.py namespace:=cpsl_uav_1 params_file:=nav2_uav.yaml"
    ...
    tmux send-keys -t $SESSION_NAME:0.8 "ros2 launch dataset_generator record_dataset.launch.py namespace:=cpsl_uav_1 param_file:=uav_dataset_radvel.yaml"
