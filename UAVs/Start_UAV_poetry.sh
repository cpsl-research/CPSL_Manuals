...
if [ $? != 0 ]; then
    ...
    tmux send-keys -t $SESSION_NAME:0.2 "nice -n -10 ros2 launch cpsl_ros2_sensors_bringup uav_sensor_bringup.launch.py lidar_enable:=false lidar_scan_enable:=true camera_enable:=false front_radar_enable:=true down_radar_enable:=true platform_description_enable:=true rviz:=false namespace:=$PLATFORM_NAME"
    ...
    sleep 1
    tmux send-keys -t $SESSION_NAME:0.5 "ros2 launch pc_processing ugv_bringup.launch.py scan_enable:=true namespace:=$PLATFORM_NAME" 
