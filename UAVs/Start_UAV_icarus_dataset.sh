...
if [ $? != 0 ]; then
    ...
    tmux send-keys -t $SESSION_NAME:0.2 "nice -n -10 ros2 launch cpsl_ros2_sensors_bringup uav_sensor_bringup_IcaRAus.launch.py lidar_enable:=false lidar_scan_enable:=false camera_enable:=false front_radar_enable:=true back_radar_enable:=true down_radar_enable:=false platform_description_enable:=true rviz:=false namespace:=$PLATFORM_NAME vicon_enable:=true"

    echo "(not working currently) setting up flow estimation..."
    ...
    echo "setting up IcaRAus Processing (GNN)"
    ...
    sleep 1
    tmux send-keys -t $SESSION_NAME:0.4 "ros2 launch pc_processing uav_IcaRAus_gnn_bringup.launch.py scan_enable:=true namespace:=$PLATFORM_NAME" 
    ...
    tmux send-keys -t $SESSION_NAME:0.8 "ros2 launch dataset_generator record_dataset.launch.py namespace:=$PLATFORM_NAME param_file:=uav_dataset_icaraus_dataset.yaml dataset_subpath:=north_1"
